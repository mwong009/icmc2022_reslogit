...
from biogeme_aesara.functions import hessians, bhhh, neg_loglikelihood
...
def correlation_matrix(model, db):
    ...
    df = pd.DataFrame(
        columns=[p.name for p in model.params if p.status != 1],
...
def gradient_norm(model, db):
    ...
    gparams = [p() for p in model.params if p.status != 1]
    grads = aet.grad(cost, gparams)
